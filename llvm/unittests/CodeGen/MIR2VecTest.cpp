...
protected:
  void SetUp() override { MIR2VecVocabTestFixture::SetUp(); }
...
TEST_F(MIR2VecEmbeddingTestFixture, VocabularyStringKeys) {
  ...
  unsigned PhyRegIndex = Vocab.getCanonicalIndexForRegisterName("GR32", true);
  std::string PhyRegKey = Vocab.getStringKey(PhyRegIndex);
  EXPECT_EQ(PhyRegKey, "PhyReg_GR32");

  unsigned VirtRegIndex = Vocab.getCanonicalIndexForRegisterName("GR32", false);
